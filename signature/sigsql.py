...
def main(argv):
    ...
    if options.verbose:
	sys.stderr.write("Opening " + options.database +"\n")
    conn = sqlite3.connect(options.database)
    conn.create_function("REGEXP", 2, regexp)
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS Signatures ( dir text, platform text, platform_version, date datetime, cancertype text,  featuretype text,  phenotypeclass text,  feature text)")
    c.execute("CREATE TABLE IF NOT EXISTS Samples ( samplename text, signature int, state int)")
    c.execute("CREATE TABLE IF NOT EXISTS Data ( signature int, name text, d_score float, falseCalls float, qValue float, pValue float, stddev float)")
    conn.commit()
