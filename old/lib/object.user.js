...
User.prototype.getPersonalCollaboration = function (){
  ...
  if (email) return ["user:" + email]; // Support multiple email addresses
  return [];
};
...
if (Meteor.isServer) {
  ...
  User.prototype.syncCollaborations = function () {
    ...
    var queuedOrCompleted = {}; // should be a Set, but Set is not availble in this revision of the Meteor/NodeJS runtime.
    ...
    Accounts.onLogin(function(user){
      if (!user.syncCollaborations) // if not wrapped in an object, wrap it.
	  user = new User(user);
