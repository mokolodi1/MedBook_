...
def npSummarizeData(sample_names, data):
    ...
    summary["sample"]["labels"] = ("Lower Quartile", "Median", "Upper Quartile",
                                   "Median of Up Alterations", "Number of Up Alterations", 
                                   "Median of Down Alterations", "Number of Down Alterations")
...
def pSummarizeData(sample_names, data, class1, class2, eps=1e-8):
    ...
    print "class1", len(class1), class1
    print "class2", len(class2), class2
    ...
    for i in range(len(sample_names)):
        ...
        if class2 == "Null":
            ...
            #    print "SKIPPING", s
    numSamples = len(sampleIndex)
    numSamplesNull = len(naIndex)
    print numSamples, "#sampleIndex", sampleIndex
    print numSamplesNull, "#naIndex", naIndex[1:10]
    #pdb.set_trace()
    #assert numSamples == len(class1)
    #assert numSamplesNull == len(class2)
...
def pPlotSummary(titleStr, imgSize, numSamples, summary, outPDF, \
        red_label, grey_label='all permuted samples'):
    entities = getEntities(summary)
    ...
    grey_title = grey_label
    if grey_label == 'Null':
        grey_title = 'Permuted'

    pylab.plot(X, Y_avg, 'r', label='Mean activity in '+red_label+' samples, shaded red is +/- 1 std. deviation')
    pylab.fill_between(X, Y_stdUp, Y_stdDn, edgecolor='r', facecolor='r', alpha=0.3)

    pylab.plot(X, Y["na"]["avg"], 'k', label='Mean activity in '+grey_title+' samples, shaded grey is +/- 1 std. deviation')
    pylab.fill_between(X, Y["na"]["up"], Y["na"]["dn"], edgecolor='k', facecolor='k', alpha=0.1)
    ...
    pylab.plot([1,maxX], [0,0], 'k:', alpha=0.8)
    pylab.xlabel("Cellular Entities (genes, activated protiens, complexes and abstract processes)")
    pylab.ylabel("Inferred Activity (z-score)")
    ...
    pylab.savefig(outPDF)
    pylab.close()


def createPlotFromData(pathwayName, imgSize, imgFilename, parametric, sample_names, data, red_label, grey_label, class1, class2):
    if parametric:
        numSamples, summary = pSummarizeData(sample_names, data, class1, class2)
        titleStr = pathwayName + " -- " + str(numSamples) + " samples"
        if (printPDF):
            pPlotSummary(titleStr, imgSize, numSamples, summary, imgFilename, red_label, grey_label)
    else:
