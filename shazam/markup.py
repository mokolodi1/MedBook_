...
class page:
    ...
    def css( self, filelist , medium='all'):
        """This convenience function is only useful for html.
        It adds css stylesheet(s) to the document via the <link> element."""
      
        if isinstance( filelist, basestring ):
            self.link( href=filelist, rel='stylesheet', type='text/css', media=medium)
        else:
            for file in filelist:
                self.link( href=file, rel='stylesheet', type='text/css', media=medium )
    ...
    def scripts( self, myobject ):
        ...
        if isinstance( myobject, dict ):
            for src, type in myobject.iteritems( ):
                if len(type) == 2:
                    # added string in between script (for document ready stuff)
                    t = type[0]  # type
                    s = type[1]  # string inside script
                    if src == '':
                        self.script(s, type='text/%s' % t )
                    else:
                        self.script(s, src=src, type='text/%s' % t )
        elif isinstance( myobject, list ):
            for src, type in myobject:
                if len(type) == 2:
                    # added string in between script (for document ready stuff)
                    t = type[0]  # type
                    s = type[1]  # string inside script
