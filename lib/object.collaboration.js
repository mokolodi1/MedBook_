...
//   Collaborations.upsert({_id: this._id}, {$set: this});
// };
 ...
 * ```
 */
// XXX: shouldn't this be in a Meteor method
...
Collaboration.prototype.addCollaborator = function (emailAddress) {
  console.log('adding...', emailAddress);
  this.collaborators.push(emailAddress);
  Meteor.call('collaboration/update', this);
};
...
// // NOTE: deprecated by Teo because not every user has a username.
// //       Feel free to put back in if this is useful.
// if (Meteor.isClient){
//
//
//
...
// XXX: I don't see why this is attached to the collaboration object
if (Meteor.isServer){
 ...
 * ```
 */
Collaboration.prototype.getAssociatedCollaborators = function () {
  //process.env.DEBUG && console.log('Collaboration.getCollaborators', JSON.stringify(this));
  ...
  for (var i = 0; i < collaborationLookupQueue.length; i++) {
    ...
    // look it up from the collection
    var collaboration = Collaborations.findOne({_id: collaborator});

    if (collaboration && collaboration.collaborators) {
      // look through the collaborators
      collaboration.collaborators.forEach(function(associatedCollaboration){
        // and then look for other nodes
        if (collaborator.indexOf("@") === -1) {
          // and add the leaves to our graph
          if (associatedCollaboration !== undefined) {
            if (collaborationLookupQueue.indexOf(associatedCollaboration) === -1) {
              collaboratorsGraph.push(associatedCollaboration);
              // collaborationLookupQueue.push(associatedCollaboration);
            }
          }
 ...
 * ```
 */
Collaboration.prototype.getAssociatedCollaborations = function () {
  //process.env.DEBUG && console.log('Collaboration.getCollaborators', JSON.stringify(this));
  ...
  for (var i = 0; i < collaborationLookupQueue.length; i++) {
    ...
    if (collaboration && collaboration.collaborators) {
      ...
      collaboration.collaborators.forEach(function(associatedCollaboration){
        // and then look for other nodes
        if (associatedCollaboration.indexOf("@") === -1) {
          // and add the leaves to our graph
          if (associatedCollaboration !== undefined) {
            // if (collaborationLookupQueue.indexOf(associatedCollaboration) === -1) {
              collaboratorsGraph.push(associatedCollaboration);
