...
def fileData(fname):
    ...
    for i in range(len(sample_names)):
        sample_name = sample_names[i].split(' ')[0]
        if sample_name in naive:
            sample_names[i] = 'na_'+sample_names[i]
            print "#naive", sample_names[i]
        elif sample_name in abi:
            print "#abi", sample_names[i]
        elif sample_name.startswith("na_"):
            print "#null", sample_names[i]
        else: 
            sample_names[i] = 'na_'+sample_names[i]
            print "#null", sample_names[i]
...
def createGeneListPage(f, parametric, uniqueName, red_label, grey_label):
    ...
    pathwayName, entitySummary, pngFile = imgFG.createPlotFromData(pathwayName, imgSize, 
                                                             ...
                                                             samples, data, red_label, grey_label)
...
def createTopPathwaysPage(pResults):
    page, summary_tsv  = initializePage(t = "Per-pathway summoray of activity", h = "")
...
def main(directory, pathway_directory, red_label, grey_label, index_html_path):
...
def usage():
    print "usage: python htmlFG.py ipl_directory pathway_directory pathway_pids.tab index.html"
...
if __name__ == "__main__":
    ...
    red_label = sys.argv[4]
    grey_label = sys.argv[5]
    index_html_path = sys.argv[6]
    initPathwayNameDict(path_file=path_list)
    #import pdb ; pdb.set_trace()
    main(directory, pathways, red_label, grey_label, index_html_path)
